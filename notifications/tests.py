...
class UserProfileAPITest(APITestCase):
    ...
    def test_update_telegram_id(self):
        response = self.client.patch('/api/notifications/profile/', {'telegram_id': '123456789'}, format='json')
        self.assertEqual(response.status_code, 200)
...
class HabitReminderTaskTest(APITestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='reminderuser', password='testpass')
    ...
    def test_reminder_sent(self, mock_send):
        ...
        mock_send.assert_called_once_with('123456789', f'Напоминание: Выпить воду в Дом ({self.habit.time.strftime("%H:%M")})')
